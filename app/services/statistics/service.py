...
def get_suggestion_count(db:Session):
    query_text = text(f"""
                      SELECT 
                        count(*) as metadata_count,
                        (select count(*) from audit_data
                        ) as data_count
                      FROM {models.AuditMetadata.__tablename__} 
                      """)
    data = db.execute(query_text, bind_arguments={"bind": engine_audit}).first()
    result = {
        "metadata_count": data[0],
        "data_count": data[1],
    }
    return(result)
